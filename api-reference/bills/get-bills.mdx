...
<ParamField query="account_id" type="string">
  Optional field specifying the `EnergyAccount` to query `Bill`s for. If omitted, `Bill`s for
...
<ResponseField name="account_id" type="string">
  ID of `EnergyAccount` that bills belong to
</ResponseField>

<ResponseField name="bills" type="array of objects">
  `Bill` objects for the `EnergyAccount`
  <Expandable>
